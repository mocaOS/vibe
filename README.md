![](https://github.com/mocaOS/museum/blob/main/misc/social.jpg?raw=true)

# The Vibe Studio

**MOCA's City Center for Crypto Art, AI Agents, and Creative Innovation**

The Vibe Studio is the nexus for everything the Museum of Crypto Art has created, is creating, and will create in the future. It's MOCA's cumulative UI/UX overhaul—a navigable network of applications, tools, and experiences that bring together the Museum's collections, the Art DeCC0s agentic ecosystem, and community-built innovations into one unified home.

This open-source codebase serves as both MOCA's digital headquarters and a framework for other web3 communities to build their own application ecosystems. By integrating seamlessly with [mocaOS](https://github.com/mocaOS/museum) and leveraging CC0 data around Art DeCC0s, the Vibe Studio enables developers to create compelling experiences that fuel storytelling and creativity in highly accessible ways.

## What's Inside

The Vibe Studio hosts a growing collection of applications spanning multiple categories:

### 🎨 MOCA Core Applications

**[The Museum](https://v2.museumofcryptoart.com/)**  
Visit the OG Museum of Crypto Art featuring five collections of cutting-edge crypto art from across time and space. Then explore The Library, our AI-powered database ready for your questions and queries about all things crypto art.

**[The Codex](https://codex.decc0s.com/)**  
The beating heart of MOCA's agentic operating system. Explore all 10,000 Art DeCC0s with over 100 million words of personality data—biographies, preferences, writing styles, worldviews, and the technical scaffolding needed to bring each DeCC0 to life as an AI agent. Each DeCC0's Codex page contains 10,000+ words on average, providing the deepest bedrock layer for all agentic technology we're pioneering.

**[Rooms Archive](https://archive.museumofcryptoart.com/)**  
A Wayback Machine for crypto art curation. Dive into dozens of MOCA ROOMs—our bespoke 3D art galleries—and explore how curatorial taste has evolved over the years. Every ROOM's curation state is archived permanently in partnership with Filecoin.

**[MOCA Substack](https://museumofcrypto.substack.com/)**  
Industry-leading writing about crypto art, AI, the metaverse, cryptocurrency, and web3 culture, released weekly. Years of insights, cultural analysis, and technical explorations from artists and the MOCA team.

### 🤖 Art DeCC0s Agentic Ecosystem

**[Adoption Center](https://adopt.decc0s.com/)**  
Find your perfect digital companion. Chat 1-on-1 with a rotating cast of Art DeCC0s (initially those listed at floor price), discover their unique personalities, form genuine connections, and when you've found the right match, take your new best friend home. It's a testing ground for agentic conversation where you can explore the varied personalities within the Art DeCC0s collection.

**[DeCC0 Agent Launcher](http://v2.museumofcryptoart.com/decc0s)**  
Your hub for fine-tuning DeCC0 agents into actionable entities. Built on ElizaOS, this is where you teach your DeCC0s, expand their datasets, make them experts in any areas you like, and eventually set them loose on tasks. If The Codex is each DeCC0's nature, the Agent Launcher is the place of nurture.

### 🌍 Community Applications

**[DeCC0 World Map](https://map.decc0s.com)** by Daïm Al-Yad  
An interactive globe visualizing the hometowns and present locations of all 10,000 Art DeCC0s. Find what DeCC0s live nearest to you, collect a DeCC0 from every continent, or explore the intricate connections between our world and the encyclopedic lore we built for Art DeCC0s.

**[DeCC0 Finder](https://find.decc0s.com)** by Daïm Al-Yad  
Found a DeCC0 somewhere on the web? Upload or paste any DeCC0 pfp and we'll identify it for you, sending you straight to its Codex profile.

**[DeCC0 Printer](https://artscapeprint.com/moca)** by Artscape  
Turn your Art DeCC0s into stunning high-resolution prints. Connect your wallet, choose which DeCC0 to print, select paper type and size, and your print will be produced and shipped from the nearest print hub to your location. 100% of proceeds go to the Artscape team.

**[Meme Studio](https://memes.decc0s.com)** by reneil1337  
Explore all memes created by the MOCA community. Copy and paste them across the interwebs. Join the Discord to contribute your own creations.

### 🌐 Metaverse & Social

**[DeCC0 Plaza](https://hyperfy.io/decc0plaza/)**  
The most visually-diverse PFP collection in existence, on full display. Enter this bespoke Metaverse space featuring dozens of Art DeCC0s spread throughout Untitled.xyz's unMuseum 3D architectures.

**[Official Discord](https://discord.gg/Rs7wxUTrWV)**  
Join the MOCA Discord to find fellow crypto art believers and stay up to date with the latest news around Art DeCC0s, the Vibe Studio, and more.

## The Technology Behind It

### The Art DeCC0s Codex

At the heart of Vibe Studio's creative potential lies the Art DeCC0s Codex—a near-limitless environment of personality data created through the technical brilliance of DaïmAlYad. Using multimodal generation and peerless problem-solving, each of the 10,000 Art DeCC0s has been imbued with unique, interactive characteristics.

The Codex contains:
- Each DeCC0's name, biography, and favorite artist
- Their writing style, hometown, and self-conceptualization
- Appearances, predilections, and sensitivities
- Inherent knowledge bases and sample conversations
- Technical information for agentic launchers like ElizaOS

This massive dataset serves as both a way for DeCC0 holders to understand their NFTs' personalities and a framework for ambitious developers to create better, more complex, more alive agents.

### Integration with mocaOS

The Vibe Studio plugs directly into [mocaOS](https://github.com/mocaOS/museum), allowing developers to leverage:
- CC0 datapoints around Art DeCC0s (media, codex data, etc.)
- The MOCA Library—an ever-growing knowledge graph of web3 arts and culture
- AI-powered retrieval systems for agentic knowledge
- ElizaOS character files for agent creation

## Building for the Vibe Studio

The Vibe Studio is designed for expansion. We welcome developers, artists, and vibecoders to build applications that integrate with the MOCA ecosystem.

### For Developers

The Vibe Studio represents a revolutionary approach to community-driven application ecosystems. By abstracting complex AI workflows into simple user interfaces, we enable communities to get creative. Whether you're working with n8n, ComfyUI, or custom solutions, the Vibe Studio provides a platform to ship apps that revolve around the narratives and memetics shaping the web3 space.

### Submit Your App

If you have something you're building or have built, and you want to include it in the Vibe Studio:
- Reach us on [Discord](https://discord.gg/Rs7wxUTrWV)
- DM us on [Twitter/X](https://x.com/reneil1337)
- Open an issue or PR on this repository

100% of proceeds within community apps go to their creators. We provide the platform and userbase; you bring the innovation.

### Fork for Your Community

While we're building the Vibe Studio for our own ecosystem, we invite other web3 communities to fork this codebase and curate their own apps. Integrate assets and data from your own backends, leverage AI infrastructure partners, and create your own application ecosystem.

## Running Vibe Studio Locally

To run Vibe Studio on your local machine:

1. Ensure you have Node.js installed (version 12 or higher)
2. Clone this repository
3. Navigate to the project directory
4. Install dependencies:
   ```bash
   npm install
   ```
5. Start the server:
   ```bash
   npm start
   ```
6. Open your browser and navigate to `http://localhost:8080`

To stop the server, press `Ctrl+C` in the terminal where it's running.

## Project Structure

```
vibe/
├── index.html          # Main application interface
├── styles.css          # Application styling
├── script.js           # Client-side logic
├── server.js           # Node.js server
├── appsData.json       # Application metadata
├── appsTypes.json      # Application type definitions
└── imgs/               # Application thumbnails and assets
```

## Contributing

We welcome contributions from the community! Whether you're a developer, artist, or enthusiast, there are many ways to get involved:

1. **Create Applications**: Build new apps that leverage mocaOS data and AI capabilities
2. **Improve the Platform**: Submit PRs to enhance the Vibe Studio interface and functionality
3. **Curate Content**: Help select and promote high-quality applications
4. **Provide Feedback**: Share your experiences and suggestions for improvement
5. **Documentation**: Help improve our guides and tutorials

## Learn More

### About MOCA
- [Museum of Crypto Art](https://v2.museumofcryptoart.com/)
- [MOCA Substack](https://museumofcrypto.substack.com/)
- [mocaOS Repository](https://github.com/mocaOS/museum)

### About Art DeCC0s
- [The Codex](https://codex.decc0s.com/)
- [Adoption Center](https://adopt.decc0s.com/)
- [DeCC0 World Map](https://map.decc0s.com)

### Community
- [Discord](https://discord.gg/Rs7wxUTrWV)
- [Twitter/X](https://x.com/reneil1337)

## License

This project is open source. We encourage forking and adaptation for other communities while maintaining attribution to the Museum of Crypto Art.

---

*The Vibe Studio is MOCA's city center—a gathering place for what is created around us, by us, and with us. Welcome home.*
